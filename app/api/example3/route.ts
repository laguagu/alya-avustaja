...
import { getAnswerFromExample2 } from "@/lib/ragActions";
...
export async function POST(req: Request) {
  try {
    ...
    if (
      ...
      // Kutsu example2-mallia, kun ensimmäinen malli ei löydä vastausta
      const resultFromtChain2 = await getAnswerFromExample2({
        chat_history: formattedPreviousMessages.join("\n"),
        question: currentMessageContent,
        messages,
      });

      return new Response(resultFromtChain2);
    }
